...
static SIMPLE_DEV_PM_OPS(tpm_user_emul_pm, tpm_pm_suspend, tpm_pm_resume);

static struct platform_driver user_emul_drv = {
	.driver = {
		.name		= "tpm_user_emul",
		.pm		= &tpm_user_emul_pm,
	},
...
static int tpm_user_send(struct tpm_chip *chip, u8 *buf, size_t len)
{
	...
	case TPM_USER_IDLE:
		...
		break;
	default:
		dev_err(chip->pdev, "Sending in state %u\n", state->status);
	case TPM_USER_EIO:
		kfree(pkt);
		ret = -EIO;
		break;
...
again:
	...
	if (copied != 0) {
		...
		kfree(pkt);
		dev_err(state->chip->pdev, "Emulator EFAULT in read\n");
		state->status = TPM_USER_EIO;
		ret = -EFAULT;
			      ...
			      loff_t *pos)
{
	...
	if (datalen == 0) {
		dev_err(state->chip->pdev, "Empty reply\n");
		return -EMSGSIZE;
	}

	if (datalen < TPM_HEADER_SIZE) {
		dev_err(state->chip->pdev, "Data packet missing TPM header\n");
		return -EMSGSIZE;
	}
	...
	if (expected != datalen) {
		dev_err(state->chip->pdev, "Data packet size (%zu) != expected (%x)\n",
			datalen, expected);
		ret = -EMSGSIZE;
		goto err_free;
	}
	...
	default:
		dev_err(state->chip->pdev, "Reply unexpected in state (%u)\n",
			state->status);
		ret = -EPROTO;
		break;
	}
...
static int tpm_user_recv(struct tpm_chip *chip, u8 *buf, size_t count)
{
	...
	default:
		dev_err(chip->pdev, "TPM data recv in unexpected state (%u)\n",
			state->status);
		ret = -EIO;
		break;
	}
	...
	if (pkt) {
		if (ret > 0) {
			if (pkt->size > count) {
				dev_err(chip->pdev, "Received excess data\n");
				ret = -EIO;
			} else {
...
static void tpm_user_initialiser(struct work_struct *work)
{
	...
	ret = platform_driver_register(&user_emul_drv);
	if (ret < 0)
		goto err_dev;
	pdev = platform_device_register_simple("tpm_user_emul", -1, NULL, 0);
	if (IS_ERR(pdev)) {
		platform_driver_unregister(&user_emul_drv);
		ret = PTR_ERR(pdev);
		goto err_dev;
	}
	state->pdev = pdev;
	...
	ret = -ENODEV;
	chip = tpmm_chip_alloc(&pdev->dev, &tpm_user_class);
	if (IS_ERR(chip))
		goto err_reg;
	...
	if (!(chip->flags & TPM_CHIP_FLAG_TPM2)) {
		/* Not all variants of the emulator support getting the
		 * timeout
		 */
...
static int tpm_user_release(struct inode *inode, struct file *file)
{
	...
	if (ret == 0) {
		...
		tpm_chip_unregister(state->chip);
